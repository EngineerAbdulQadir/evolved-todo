...
describe('TaskForm Component', () => {
  ...
  describe('Create Mode', () => {
    ...
    it('submit button is disabled when title is empty', () => {
      ...
      const titleInput = screen.getByPlaceholderText('ENTER DIRECTIVE...');
    ...
    it('submit button is enabled when title has value', async () => {
      ...
      const submitButton = screen.getByText('Initialize_Sequence');
    ...
    it('calls onSubmit with task data when form is submitted', async () => {
      ...
      await user.type(screen.getByPlaceholderText('ENTER DIRECTIVE...'), 'Test task');
      await user.type(screen.getByPlaceholderText('ADD CONTEXT...'), 'Test description');

      await user.click(screen.getByText('Initialize_Sequence'));
    ...
    it('clears form after successful submission', async () => {
      ...
      const titleInput = screen.getByPlaceholderText('ENTER DIRECTIVE...');
      const descriptionInput = screen.getByPlaceholderText('ADD CONTEXT...');
  ...
  describe('Edit Mode', () => {
    ...
    it('shows "Save Changes" button in edit mode', () => {
      ...
      expect(screen.getByText('Update_Protocol')).toBeInTheDocument();
      expect(screen.queryByText('Initialize_Sequence')).not.toBeInTheDocument();
    });
  ...
  describe('Priority Selection', () => {
    it('allows selecting no priority', async () => {
      ...
      render(<TaskForm onSubmit={mockOnSubmit} onCancel={mockOnCancel} />);
      expect(screen.getByDisplayValue('CLASS 2 (MEDIUM)')).toBeInTheDocument();
    });
    ...
    it('allows selecting low priority', async () => {
      ...
      const prioritySelect = screen.getByDisplayValue('CLASS 2 (MEDIUM)');
      await user.selectOptions(prioritySelect, 'low');

      expect(screen.getByDisplayValue('CLASS 3 (LOW)')).toBeInTheDocument();
    });
    ...
    it('allows selecting high priority', async () => {
      ...
      expect(screen.getByDisplayValue('CLASS 1 (HIGH)')).toBeInTheDocument();
    });
  });
  ...
  describe('Loading States', () => {
    it('shows "Creating..." while submitting in create mode', async () => {
      ...
      // Button text is static: "Initialize_Sequence"
      render(<TaskForm onSubmit={mockOnSubmit} onCancel={mockOnCancel} />);
      expect(screen.getByText('Initialize_Sequence')).toBeInTheDocument();
    });
