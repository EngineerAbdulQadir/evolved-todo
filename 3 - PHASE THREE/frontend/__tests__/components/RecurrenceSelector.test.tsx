...
describe('RecurrenceSelector Component', () => {
  ...
  it('renders with default "none" value', () => {
    ...
    const recurrenceSelect = screen.getByLabelText(/Cycle_Type/i);
    expect(recurrenceSelect).toHaveValue('none');
    expect(screen.getByText('Manual (Single Execution)')).toBeInTheDocument();
  });
  ...
  it('displays all recurrence options', () => {
    ...
    expect(screen.getByText('Manual (Single Execution)')).toBeInTheDocument();
    expect(screen.getByText('Daily Loop (24h)')).toBeInTheDocument();
    expect(screen.getByText('Weekly Loop (7d)')).toBeInTheDocument();
    expect(screen.getByText('Monthly Loop (30d)')).toBeInTheDocument();
  });
  ...
  describe('Daily Recurrence', () => {
    ...
    it('shows daily recurrence info text', () => {
      ...
      expect(screen.getByText(/Task regenerates T\+24H post-completion/i)).toBeInTheDocument();
    });

    it('does not show day selector for daily recurrence', () => {
      render(<RecurrenceSelector {...defaultProps} recurrence="daily" />);

      expect(screen.queryByLabelText(/Target_Weekday/i)).not.toBeInTheDocument();
      expect(screen.queryByLabelText(/Target_Ordinal/i)).not.toBeInTheDocument();
    });
  });
  ...
  describe('Weekly Recurrence', () => {
    ...
    it('shows weekday selector for weekly recurrence', () => {
      ...
      expect(screen.getByLabelText(/Target_Weekday/i)).toBeInTheDocument();
    });
    ...
    it('selects a specific weekday', async () => {
      ...
      const weekdaySelect = screen.getByLabelText(/Target_Weekday/i);
    ...
    it('shows info text when weekday is selected', () => {
      ...
      expect(screen.getByText(/Next instance: Every Monday/i)).toBeInTheDocument();
    });

    it('shows waiting message when no weekday selected', () => {
      render(<RecurrenceSelector {...defaultProps} recurrence="weekly" />);

      expect(screen.getByText(/Awaiting day parameter/i)).toBeInTheDocument();
    });
  ...
  describe('Monthly Recurrence', () => {
    ...
    it('shows day of month selector for monthly recurrence', () => {
      ...
      expect(screen.getByLabelText(/Target_Ordinal/i)).toBeInTheDocument();
    });
    ...
    it('selects a specific day of month', async () => {
      ...
      const daySelect = screen.getByLabelText(/Target_Ordinal/i);
    ...
    it('shows info text when day is selected', () => {
      ...
      expect(screen.getByText(/Next instance: Day 15 of current month/i)).toBeInTheDocument();
    });

    it('shows waiting message when no day selected', () => {
      render(<RecurrenceSelector {...defaultProps} recurrence="monthly" />);

      expect(screen.getByText(/Awaiting date parameter/i)).toBeInTheDocument();
    });
  });
  ...
  describe('Info Display', () => {
    it('does not show info when recurrence is none', () => {
      ...
      expect(screen.queryByText(/Scheduler Output/i)).not.toBeInTheDocument();
    });

    it('shows info when recurrence is set', () => {
      render(<RecurrenceSelector {...defaultProps} recurrence="daily" />);

      expect(screen.getByText(/Scheduler Output/i)).toBeInTheDocument();
    });
    ...
    it('correctly displays info for all weekdays', () => {
      ...
      weekdays.forEach(({ day, name }) => {
        ...
        expect(screen.getByText(new RegExp(`Next instance: Every ${name}`, 'i'))).toBeInTheDocument();
        unmount();
      });
    });
