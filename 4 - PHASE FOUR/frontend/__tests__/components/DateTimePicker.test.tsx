...
describe('DateTimePicker Component', () => {
  ...
  describe('Date Selection', () => {
    it('renders date input field', () => {
      ...
      expect(screen.getByLabelText(/Target_Date/i)).toBeInTheDocument();
    });

    it('allows selecting a date', async () => {
      render(<DateTimePicker {...defaultProps} />);

      const dateInput = screen.getByLabelText(/Target_Date/i);
  ...
  describe('Time Selection', () => {
    it('does not show time input when no date is set', () => {
      ...
      expect(screen.queryByLabelText(/T-Minus.*Time/i)).not.toBeInTheDocument();
    });

    it('shows time input when date is set', () => {
      render(<DateTimePicker {...defaultProps} dueDate="2025-12-31" />);

      expect(screen.getByLabelText(/T-Minus.*Time/i)).toBeInTheDocument();
    });

    it('allows selecting a time', async () => {
      render(<DateTimePicker {...defaultProps} dueDate="2025-12-31" />);

      const timeInput = screen.getByLabelText(/T-Minus.*Time/i);
  ...
  describe('Error Handling', () => {
    ...
    it('shows error icon when error is present', () => {
      ...
      expect(errorSection).toHaveClass('text-red-500');
    });
  });
  ...
  describe('Integration', () => {
    ...
    it('shows time input immediately when both date and time are provided', () => {
      ...
      expect(screen.getByLabelText(/T-Minus.*Time/i)).toBeInTheDocument();
      expect(screen.getByLabelText(/T-Minus.*Time/i)).toHaveValue('14:00');
    });
  });
  ...
  describe('Styling and Accessibility', () => {
    it('has proper labels for screen readers', () => {
      ...
      expect(screen.getByLabelText(/Target_Date/i)).toHaveAttribute('type', 'date');
      expect(screen.getByLabelText(/T-Minus.*Time/i)).toHaveAttribute('type', 'time');
    });
