...
export function useTasks(params: UseTasksParams | string | null): UseTasksReturn {
  ...
  useEffect(() => {
    ...
    // Listen for task updates from AI chat
    const handleTaskUpdate = () => {
      console.log('🔄 Task update event received - refreshing tasks');
      fetchTasks();
    };

    window.addEventListener('ai-task-updated', handleTaskUpdate);

    return () => {
      window.removeEventListener('ai-task-updated', handleTaskUpdate);
    };
  }, [fetchTasks]);
