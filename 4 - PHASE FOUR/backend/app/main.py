"""
Main FastAPI application entry point with CORS configuration.

This module initializes the FastAPI app, configures middleware,
and sets up routing for the Evolved Todo backend API.
"""

# CRITICAL: Fix Windows encoding issues by forcing UTF-8 for all I/O
import sys
import io
import logging

# Reconfigure stdout and stderr to use UTF-8 encoding
if sys.platform == "win32":
    sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
    sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding='utf-8', errors='replace')

# Configure logging to use UTF-8 encoding
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.StreamHandler(sys.stdout)
    ]
)

from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware

from app.core.settings import settings

app = FastAPI(
    title="Evolved Todo API",
    description="""
    # Evolved Todo Backend API

    Production-ready REST API for task management with multi-user support.

    ## Features

    - **Authentication**: JWT-based authentication with Better Auth
    - **Task Management**: Full CRUD operations for tasks
    - **Advanced Filtering**: Search, sort, and filter tasks by multiple criteria
    - **Due Dates & Reminders**: Track task deadlines with visual indicators
    - **Recurring Tasks**: Automatic task creation for daily, weekly, monthly patterns
    - **Priorities & Tags**: Organize tasks with priorities and custom tags
    - **Multi-User**: Complete data isolation between users

    ## Rate Limiting

    - **Limit**: 100 requests per minute per user
    - **Headers**: `X-RateLimit-Limit`, `X-RateLimit-Remaining`
    - **Status**: 429 when limit exceeded

    ## Authentication

    Most endpoints require authentication via JWT token in the Authorization header:

    ```
    Authorization: Bearer <jwt_token>
    ```

    Get a token by calling `/api/auth/login` or `/api/auth/register`.

    ## Error Responses

    All errors follow this format:

    ```json
    {
      "detail": "Error message",
      "status_code": 400
    }
    ```

    Common status codes:
    - `200`: Success
    - `201`: Created
    - `400`: Bad Request
    - `401`: Unauthorized
    - `403`: Forbidden
    - `404`: Not Found
    - `429`: Rate Limit Exceeded
    - `500`: Internal Server Error
    """,
    version="2.0.0",
    debug=settings.debug,
    contact={
        "name": "Evolved Todo Support",
        "url": "https://github.com/yourusername/evolved-todo",
    },
    license_info={
        "name": "MIT License",
        "url": "https://opensource.org/licenses/MIT",
    },
    docs_url="/api/docs",  # Swagger UI
    redoc_url="/api/redoc",  # ReDoc
    openapi_url="/api/openapi.json",  # OpenAPI schema
)

# Configure CORS for frontend origin
# Allow both with and without trailing slash for development
allowed_origins = [
    settings.frontend_url,
    settings.frontend_url.rstrip("/"),  # Remove trailing slash if present
    "http://localhost:3000",  # Explicit localhost
]
print(f"[CORS] Allowed origins: {allowed_origins}")

app.add_middleware(
    CORSMiddleware,
    allow_origins=allowed_origins,
    allow_credentials=True,
    allow_methods=["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"],
    allow_headers=["*"],
    expose_headers=["*"],
)

# Add rate limiting middleware (T225)
from app.middleware.rate_limit import rate_limit_middleware
app.middleware("http")(rate_limit_middleware)

# Register API routers
from app.api import auth, chat, health, tasks, chatkit

app.include_router(auth.router, prefix="/api/auth")
app.include_router(tasks.router, prefix="/api")
app.include_router(health.router, prefix="/api")  # Health check at root level
app.include_router(chat.router, prefix="/api")  # Phase 3: AI chat endpoint
app.include_router(chatkit.router, prefix="/api")  # Phase 3: ChatKit integration
# DISABLED: Multi-tenant features
# from app.api import organizations, invitations, teams
# app.include_router(organizations.router, prefix="/api")
# app.include_router(invitations.router, prefix="/api")
# app.include_router(teams.router, prefix="/api")


@app.on_event("startup")
async def startup_event() -> None:
    """Initialize services on application startup."""
    logger.info("=" * 80)
    logger.info("STARTUP EVENT - TESTING LOGGING")
    logger.info("=" * 80)

    # Initialize OpenAI agent
    if settings.openai_api_key:
        from app.agents.todo_agent import initialize_agent
        from app.mcp.server import mcp_server
        from app.mcp.tools.registry import initialize_all_tools

        agent = initialize_agent(settings.openai_api_key)
        mcp_tools = initialize_all_tools(mcp_server, agent)
        print(f"[OK] OpenAI agent initialized with {len(mcp_tools)} MCP tools (model: gpt-4o-mini)")
        logger.info(f"[OK] OpenAI agent initialized with {len(mcp_tools)} MCP tools (model: gpt-4o-mini)")
    else:
        print("[WARNING] OPENAI_API_KEY not set - chat endpoint will not work")
        logger.warning("[WARNING] OPENAI_API_KEY not set - chat endpoint will not work")


@app.get("/")
async def root() -> dict[str, str]:
    """Root endpoint for health check."""
    return {"message": "Evolved Todo API is running", "version": "2.0.0"}


import logging
from fastapi import Request
from fastapi.responses import JSONResponse

from app.utils import safe_str


# Set up logger for the main app
logger = logging.getLogger(__name__)


@app.exception_handler(Exception)
async def global_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    """
    Global exception handler for unhandled errors.

    Logs the error and returns a generic 500 response.
    """
    # In production, log to monitoring service (e.g., Sentry)
    # Strip emojis from exception message for Windows console compatibility
    error_msg = safe_str(exc)
    logger.error(f"Unhandled error: {error_msg}", exc_info=True)  # FIXED: Show full traceback for debugging
    return JSONResponse(
        status_code=500,
        content={"detail": "Internal server error", "status_code": 500},
    )
