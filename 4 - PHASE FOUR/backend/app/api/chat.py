...
async def fetch_conversation_history(
    ...
    # Filter out TOOL messages and intermediate assistant messages for OpenAI
    # OpenAI requires TOOL messages to follow assistant messages with tool_calls,
    # but we don't store the tool_calls field, so we exclude them from history
    filtered_messages = []
    for msg in messages:
        # Skip TOOL role messages (they're internal to our system)
        if msg.role == MessageRole.TOOL:
            continue

        # Skip intermediate assistant messages with "[Tool calls:" marker
        if msg.role == MessageRole.ASSISTANT and "[Tool calls:" in msg.content:
            continue

        # Include USER and final ASSISTANT messages
        filtered_messages.append({"role": msg.role.value.lower(), "content": msg.content})
...
async def chat(
    ...
    try:
        ...
        # CRITICAL FIX: Persist user message FIRST before agent processing
        # This ensures conversation history includes this message for next request
        user_msg = Message(
            conversation_id=conversation.id,
            user_id=user_id,
            role=MessageRole.USER,
        ...
        # CRITICAL FIX: Only persist FINAL assistant message
        # User message, tool call message, and tool results already persisted above
        ...
        await session.commit()  # Commit all messages
...
async def get_conversation_history(
    ...
    # Filter out intermediate tool messages for display
    # Only show user messages and final assistant messages (not tool calls or tool results)
    display_messages = []
    for msg in messages:
        # Skip tool result messages
        if msg.role == MessageRole.TOOL:
            continue
        # Skip assistant messages with "[Tool calls:" (intermediate messages)
        if msg.role == MessageRole.ASSISTANT and "[Tool calls:" in msg.content:
            continue
        # Include user messages and final assistant messages
...
async def list_conversations(
    ...
    Returns:
        List of conversations with IDs and metadata

    Raises:
        HTTPException: 403 if trying to access other users' conversations
    """
    ...
    conversation_list = [
        {
            ...
            "title": conv.title,  # Include title for display
...
async def update_conversation_title(
    ...
    if current_user.sub != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Cannot update other users' conversations",
        )
...
async def delete_conversation(
    ...
    if current_user.sub != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Cannot delete other users' conversations",
        )
    ...
    # SQLModel with cascade should handle message deletion, but explicit delete for clarity
